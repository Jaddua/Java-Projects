...
public class GintBase {
	...
	public List<Gint> longDiv(Gint v, Gint w) {
		List<Gint> ans = new ArrayList<Gint>();
		Gint pow = Gint.Z;
		Gint gcd = v.gcd(w);
	...
	public String longDivToString(List<Gint> num) {
		...
		String ans = convert(num.get(0)).stream().map(z -> ((Gint) z).toString()).collect(Collectors.joining(" ")) + " . ";
